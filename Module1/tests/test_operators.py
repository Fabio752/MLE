...
from .strategies import assert_close, small_floats, sigmoid_range_floats
...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert sigmoid(a) > 0 and sigmoid(a) < 1.0
...
def test_other(a: float, b: float) -> None:
    ...
    Throughout tests I am using a**2 to ensure that the input is positive.
    add
    - test symmetric
    eq
    - test symmetric
    - test sign sensitivity
    inv
    - test strictly increasing for a < 0
    - test strictly decreasing for a > 0
    inv_back
    - test strictly decreasing for a < 0
    - test strictly increasing for a > 0
    log_back
    ...
    assert eq(a, a)
    assert not eq(a, -a) or a == 0.0

    # ensure no division by zero in testing.
    if is_close(a, 0.0):
        a += 1

    assert inv(-(a**2)) < inv(-(a**2 + 1))
    assert inv(a**2) > inv(a**2 + 1)

    assert inv_back(a**2, 1) < inv_back(a**2 + 1, 1)
    assert inv_back(a**2, -1) > inv_back(a**2 + 1, -1)

    assert log_back(a**2, -1) < log_back(a**2 + 1, -1)
    assert log_back(a**2, 1) > log_back(a**2 + 1, 1)

    assert not lt(a, a)
    assert not (lt(a, a + 1) == lt(a + 1, a))

    assert max(a, b) == max(b, a)
    assert not max(a**2, a**2 + 1) == abs(max(-(a**2), -(a**2 + 1)))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert sum(ls1) + sum(ls2) == reduce(add, 0.0)(ls1) + reduce(add, 0.0)(ls2)
