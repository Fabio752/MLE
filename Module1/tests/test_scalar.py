...
def test_simple(a: float, b: float) -> None:
    ...
    # Simple EQ
    c = Scalar(a) == Scalar(b)
    assert_close(c.data, minitorch.operators.eq(a, b))

    # Simple LT
    c = Scalar(a) < Scalar(b)
    assert_close(c.data, minitorch.operators.lt(a, b))

    # Simple Exp
    c = Scalar(a).exp() + Scalar(b).exp()
    assert_close(c.data, minitorch.operators.exp(a) + minitorch.operators.exp(b))

    # Simple Inv ensure a and b are non-zero
    c = 1 / Scalar(a**2 + 1) + 1 / Scalar(-(b**2) - 1)
    assert_close(
        c.data,
        minitorch.operators.inv(a**2 + 1) + minitorch.operators.inv(-(b**2) - 1),
    )

    # Simple Log - ensure a and b are non-negative
    c = Scalar(abs(a)).log() + Scalar(abs(b)).log()
    assert_close(
        c.data, minitorch.operators.log(abs(a)) + minitorch.operators.log(abs(b))
    )

    # Simple Neg
    c = -Scalar(a) + (-Scalar(b))
    assert_close(c.data, minitorch.operators.neg(a) + minitorch.operators.neg(b))
