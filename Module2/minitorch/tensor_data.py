...
def to_index(ordinal: int, shape: Shape, out_index: OutIndex) -> None:
    ...
    """
    cur_pos = ordinal
    for i, sh in reversed(list(enumerate(shape))):
...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    diff_shape = len(max_shape) - len(min_shape)
    new_shape = list(max_shape)

    for i, (min_sh, max_sh) in enumerate(zip(min_shape, max_shape[diff_shape:])):
        if min_sh != max_sh and min_sh != 1 and max_sh != 1:
            raise IndexingError("Broadcast not possible!")
        else:
            new_shape[diff_shape + i] = max(min_sh, max_sh)
